  ...
  navItems.forEach(item => {
    if (item.dataset.view !== 'today-view') {
      item.addEventListener('click', async () => {
        ...
        // 如果切换到待办页面，刷新待办数据
        if (item.dataset.view === 'todo-view') {
...
function updateTodayDate() {
  ...
  if (todayDateEl) {
    ...
    const dateStr = today.toLocaleDateString(locale, dateOptions);
    const timeStr = today.toLocaleTimeString(locale, timeOptions);
...
async function loadTodayTodos() {
  try {
    ...
    // 数据已经在dataManager中按截止日期排序了，这里不需要再排序
